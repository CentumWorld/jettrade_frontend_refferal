...
function MemberForgetPassword(props) {
  ...
  const sendOtpFunction = () => {
    ...
    if (inputValue === "") {
      ...
      axios
        .post(`${apiurl}` + "member/member-forget-password", data)
        .then((res) => {
  ...
  const verifyOtpFunction = () => {
    ...
    if (otpValue === "") {
      ...
      axios
        .post(`${apiurl}` + "/member/member-verify-otp", data)
        .then((res) => {
  ...
  const resetPasswordFunction = () => {
    ...
    if (newPassword.password.length < 8) {
      ...
      if (newPassword.password === "") {
        ...
        axios
          .post(`${apiurl}` + "/member/member-reset-password", data)
          .then((res) => {
            message.success(res.data.message);
            handleCloseModal();
          })
      ...
      >
        <div>
          <Input
            ...
            onChange={forgetPasswordFuction}
